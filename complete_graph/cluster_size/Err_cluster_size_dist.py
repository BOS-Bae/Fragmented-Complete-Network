...
if (len(sys.argv) < 4):
    print("python3 cluster_size_diff.py N n_run n_sample sl")
    exit(1)
...
for n in range(n_s):
    ...
    if (os.path.getsize(file) > 0): 
        ...
        if (len(image) == N*n_run):
            for n_it in range(n_run):
                image_it = image[int(n_row_i):int(n_row_f)]
                n_samples += 1
                c_dist_n = get_cluster_size_distribution(N, image_it)
                c_dist_tot.append(c_dist_n)

                n_row_i += N
                n_row_f += N
        else : not_completed_arr.append(n)
    else : not_completed_arr.append(n)
...
plt.plot(x_arr, c_dist/N, label="L{}, n={}, numerical data".format(rule_num, n_run), marker = 'o', color='blue')
...
plt.figure(figsize=(10,3.5))
plt.plot(x_arr, cumul_dist/N, label="L{}, n={}, numerical data, F(M <= X)".format(rule_num, n_run), marker = 'o', color='blue')
