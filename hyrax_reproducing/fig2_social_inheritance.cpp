...
void dat_print(vector<double> &result_arr, vector<double> &dat_avg_stderr, int n_s) {
	double avg = 0;
	double std_err = 0;
	double std_d = 0;
...
int main(int argc, char *argv[]) {
	if (argc < 3) {
		cout << "./social_inheritance N MCS n_s \n";
		exit(1);
	}
	...
	int MCS = atoi(argv[2]);
	int n_s = atoi(argv[3]);
	for (double pr = 0.0; pr <= 1.05; pr += 0.05) {
		for (double pn = 0.0; pn <= 1.05; pn += 0.05) {
			...
			dat_print(result_arr, dat_avg_stderr, n_s);
